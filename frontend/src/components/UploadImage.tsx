...
export function UploadImage({ onImageAdded, image }: {
    ...
    async function onFileSelect(e: any) {
        ...
        try {
            ...
            console.log(`presigned url link ${backendUrl} and ${process.env.NEXT_PUBLIC_BACKEND_URL}`);
            
            const response = await axios.get(`${backendUrl}/user/presignedUrl`, {
                headers: {
            ...
            onImageAdded(`${cloudFrontUrl}/${response.data.fields["key"]}`);
        } catch (e) {
            console.log(e)
        }
        setUploading(false);
